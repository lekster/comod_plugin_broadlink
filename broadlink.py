...
def setup(ssid, password, security_mode):
    ...
    starttime = time.time()
    cs.sendto(payload, ('255.255.255.255', 80))
    print(1)
    timeout = 5  
    if timeout is None:
        response = cs.recvfrom(1024)
...
class device:
  ...
  def send_packet(self, command, payload):
    ...
    print (':'.join(format(x, '02x') for x in bytearray(payload)))
    aes = AES.new(bytes(self.key), AES.MODE_CBC, bytes(self.iv))
...
#import binascii
#binascii.hexlify(bytearray(devices[0].mac))


# In[40]:

#print (':'.join(format(x, '02x') for x in devices[0].mac))


# In[100]:

#devices[0].check_power_r()
